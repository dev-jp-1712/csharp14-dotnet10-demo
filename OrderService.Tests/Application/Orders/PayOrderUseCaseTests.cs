...
public class PayOrderUseCaseTests
{
    ...
    private readonly Mock<ILogger<PayOrderUseCase>> _loggerMock;
    private readonly PayOrderUseCase _useCase;

    public PayOrderUseCaseTests()
    {
        _orderRepositoryMock = new Mock<IOrderRepository>();
        _loggerMock = new Mock<ILogger<PayOrderUseCase>>();
        _useCase = new PayOrderUseCase(_orderRepositoryMock.Object, _loggerMock.Object);
    }

    [Fact]
    public async Task ExecuteAsync_WithConfirmedOrder_RecordsPayment()
    {
        // Arrange
        var orderId = Guid.NewGuid();
        var order = CreateConfirmedOrder();
        var request = new PayOrderRequest("PAY-123456");
    ...
    [Fact]
    public async Task ExecuteAsync_WithPendingOrder_ThrowsInvalidOrderTransitionException()
    {
        // Arrange
        var orderId = Guid.NewGuid();
    ...
    [Fact]
    public async Task ExecuteAsync_WithAlreadyPaidOrder_ThrowsInvalidOrderTransitionException()
    {
        // Arrange
        var orderId = Guid.NewGuid();
